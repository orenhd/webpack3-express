...
var user_login = exports.user_login = function user_login(req, res, next) {
	_user.UserModel.findOne({ username: req.body.username }).select('+password').exec(function (err, user) {
		if (err) {
			next(err);
		} else if (user) {
      ...
      this is the only property I include in the token for identification of theu user who submitted the request */
					var token = _jsonwebtoken2.default.sign({ _id: user._id }, _config2.default.app.at_string, {
						expiresIn: _config2.default.app.jwt_life_span
					});
...
var user_get = exports.user_get = function user_get(req, res, next) {
	Promise.all([_user.UserModel.findOne({ _id: req.decoded._id }), // retrieve the user who submitted the request
	_user.UserModel.findOne({ 'username': req.params.username }) // retrieve the requested user
	]).then(function (_ref) {
		...
		if (user && member) {
			var isFriend = member.friends && member.friends.length && member.friends.includes(user._id.toString()) ? true : false; // verify friendship
			res.status(200).json({ success: true, data: _extends({}, member.toObject(), { isFriend: isFriend }) });
		} else {
			next();
		}
	}).catch(function (err) {
